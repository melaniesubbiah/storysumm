...
We have corrected typos in two sentence-level faithfulness labels in ``storysumm.json`` and ``storysumm_w_subj.json``. We additionally want to clarify the language in the paper to specify that, for the gold set of labels, we merge unfaithful labels that *all three* annotators agree on, with the adjudicated expert labels, and filtered hybrid labels. We are also adding the original annotation file from the annotators with their explanations in case it is helpful for future work.
